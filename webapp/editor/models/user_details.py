...
class Profile(models.Model):
    ...
    @staticmethod
    def load(request, received_data):
        ...
        if profile == None:
            ...
            pref_string = profile.preferences
        try:
            pref_json = json.loads(pref_string)
        except:
            pref_json = json.loads("{}")
        ...
        send_data['predefined'] = json.dumps(Profile.get_predefined())
        return send_data

    @staticmethod
    def store(request, received_data):
        ...
        request.user.profile.preferences = json.dumps(prefs)
        request.user.save()
        send_data = {}
        send_data['status'] = "ok"
        return send_data
